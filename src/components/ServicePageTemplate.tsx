    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-center bg-brand-bg overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          ...
          <h1 className="font-oswald text-4xl sm:text-5xl lg:text-6xl font-bold text-brand-text leading-tight max-w-3xl">
            {heroTitle.split('|').map((part, i) => (
              i % 2 === 0
                ? <span key={i}>{part}</span>
                : <span key={i} className="text-brand-gold">{part}</span>
            ))}
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {scopeItems.map((item) => {
              ...
              return (
                <div
                  key={item.title}
                  className="bg-brand-bg border border-brand-border p-6 hover:border-brand-gold transition-colors duration-200 group"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-brand-muted font-lato leading-relaxed">
                Medical facility construction isn't like standard commercial build-out. It requires deep knowledge of
                infection control, equipment clearances, patient flow, and regulatory compliance.
                Qamar brings all of that expertise under one roof.
              </p>
              {compliance && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {features.map((f) => (
                <div key={f.title} className="bg-brand-bg border border-brand-border p-5 hover:border-brand-gold transition-colors duration-200">
                  <div className="gold-divider my-0 mb-3 w-8" />
